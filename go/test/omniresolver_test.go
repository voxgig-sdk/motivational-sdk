...
import (
	...
	omni "github.com/voxgig-sdk/motivational-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.MotivationalError); is {
				out := map[string]any{
					"name":    "MotivationalError",
...
func omniCtx(arg any, client *sdk.MotivationalSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.MotivationalSDK); is {
			client = live
