...
class LanguageDirectTest extends TestCase
{
    public function test_direct_list_language(): void
    {
        $setup = language_direct_setup([
            ["id" => "direct01"],
            ["id" => "direct02"],
        ]);
        [$_shouldSkip, $_reason] = Runner::is_control_skipped("direct", "direct-list-language", $setup["live"] ? "live" : "unit");
        if ($_shouldSkip) {
            $this->markTestSkipped($_reason ?? "skipped via sdk-test-control.json");
            return;
        }
        if ($setup["live"]) {
            foreach (["language01"] as $_liveKey) {
...
function language_direct_setup($mockres)
{
