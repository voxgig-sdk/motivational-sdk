...
class FtCtrl extends MotivationalControl
{
...
class FtClient
{
    ...
    private MotivationalUtility $utility;

    public function __construct(MotivationalUtility $utility, array $options)
    {
    ...
    public function get_utility(): MotivationalUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (MotivationalContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public MotivationalContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (MotivationalContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = MotivationalConfig::make_feature($name);
    ...
    private static function buildUrl(MotivationalSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof MotivationalError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new MotivationalResponse($fetched);
        }
    ...
    private function fail(MotivationalContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (MotivationalContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
