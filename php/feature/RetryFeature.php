...
// Motivational SDK retry feature
...
class MotivationalRetryFeature extends MotivationalBaseFeature
{
    ...
    public function init(MotivationalContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (MotivationalContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(MotivationalContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(MotivationalContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
