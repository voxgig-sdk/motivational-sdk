...
module MotivationalUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = MotivationalResponse.new({})
    ...
    if fetch_err
      ...
      response = MotivationalResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = MotivationalResponse.new(fetched)
    else
