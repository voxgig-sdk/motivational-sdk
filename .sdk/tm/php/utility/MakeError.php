...
class MotivationalMakeError
{
    public static function call(?MotivationalContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new MotivationalContext([], null);
        }
        $op = $ctx->op ?? new MotivationalOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new MotivationalResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof MotivationalError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new MotivationalError('', $msg, $ctx);
        $sdk_err->result = ($ctx->utility->clean)($ctx, $result);
        $sdk_err->spec = ($ctx->utility->clean)($ctx, $spec);
        if ($err instanceof MotivationalError) {
