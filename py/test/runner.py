...
class MotivationalTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            MotivationalTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = MotivationalTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = MotivationalTestRunner.getenv("MOTIVATIONAL_TEST_LIVE")
        override = MotivationalTestRunner.getenv("MOTIVATIONAL_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = MotivationalTestRunner.getenv(key)
        ...
        explain = MotivationalTestRunner.getenv("MOTIVATIONAL_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["MOTIVATIONAL_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if MotivationalTestRunner._test_control is not None:
            return MotivationalTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                MotivationalTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            MotivationalTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = MotivationalTestRunner.load_test_control()
...
def load_env_local():
    MotivationalTestRunner.load_env_local()


def env_override(m):
    return MotivationalTestRunner.env_override(m)


def entity_data(v):
    return MotivationalTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return MotivationalTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return MotivationalTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return MotivationalTestRunner.load_test_control()


def live_client_options():
    return MotivationalTestRunner.live_client_options()


def live_delay_ms():
    return MotivationalTestRunner.live_delay_ms()
