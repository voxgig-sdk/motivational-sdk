// Command Motivational-cli is a CLI + REPL for the Motivational SDK,
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      MOTIVATIONAL_APIKEY (api key), MOTIVATIONAL_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: MOTIVATIONAL_APIKEY carries the API key and
	// MOTIVATIONAL_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("MOTIVATIONAL_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("MOTIVATIONAL_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewMotivationalSDK(opts)
