...
boru-driven command-line client **and** interactive REPL for the Motivational
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/motivational-cli)
make build

# 2. See usage (words, entities, env vars)
./motivational-cli --help
...
MOTIVATIONAL_BASE=https://api.example.com ./motivational-cli list language

# 6. No arguments -> interactive REPL
./motivational-cli
motivational> list language
motivational> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/motivational-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/motivational-cli list language
   ```
...
Run with no arguments to open a REPL (prompt `motivational>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `language`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/motivational-cli`. |
...
(`./motivational-cli <expr>`) and interactively (the REPL), and expressions compose the
