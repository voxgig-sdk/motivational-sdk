...
[MCP](https://modelcontextprotocol.io) server exposing the Motivational SDK as
two agent tools — `motivational_list` and `motivational_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/motivational-mcp)
make build

# 2. Provide credentials via the environment
export MOTIVATIONAL_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user motivational \
  -- /absolute/path/to/motivational-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./motivational-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// motivational_list: first page of records
{ "entity": "language" }
{ "entity": "language", "query": { } }

// motivational_load: one record by id
{ "entity": "language", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/motivational-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user motivational \
     -- "$PWD"/dist/*/motivational-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `motivational_list` and `motivational_load` tools now appear
   in new sessions. Ask the agent to *"list language using motivational"*
   and it calls `motivational_list` with `{"entity":"language"}`.
...
```sh
export MOTIVATIONAL_APIKEY=sk_live_xxx            # API key
export MOTIVATIONAL_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `motivational_list` tool
...
### Call the `motivational_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `motivational_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `motivational_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `MOTIVATIONAL_APIKEY` | API key sent with every request. |
| `MOTIVATIONAL_BASE` | Optional override of the API base URL. |
...
```sh
./motivational-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"motivational_load","arguments":{"entity":"language","query":{"id":1}}}}'
```
