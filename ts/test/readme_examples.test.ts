...
//     (\`new MotivationalSDK(...)\` / \`MotivationalSDK.test(...)\` become
//     \`MotivationalSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'MotivationalSDK'
...
const TEST_SEED = {"entity":{"language":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new MotivationalSDK(...)\` and any
// \`MotivationalSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+MotivationalSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/MotivationalSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
