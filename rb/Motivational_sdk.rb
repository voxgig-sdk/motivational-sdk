...
require_relative 'Motivational_types'


class MotivationalSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = MotivationalHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = MotivationalHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, MotivationalFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    MotivationalUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = MotivationalHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = MotivationalHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = MotivationalHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => MotivationalError.new(
        "#{op}_allow",
        "MotivationalSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = MotivationalHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = MotivationalError.new(
        "graphql_error", "MotivationalSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = MotivationalSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
