...
class LanguageEntity
  ...
  def make
    opts = @_entopts.dup
    LanguageEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = MotivationalHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = MotivationalHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  #
  # @param reqmatch [LanguageListMatch, Hash, nil] match filter (any subset of
  #   Language fields); defaults to nil, treated as an empty match that lists all.
